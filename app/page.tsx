...
import { composers, DEFAULT_COMPOSER_IDS, type Composer, type Work } from '@/lib/composers';
...
function RosterView({onStart}:{onStart:(ids:string[])=>void}){
  ...
  const selected=new Set(roster);const toggle=(id:string)=>setRoster((current)=>current.includes(id)?current.filter((item)=>item!==id):current.length<48?[...current,id]:current);
  return <section className="roster-view"><div className="stage-intro"><p className="kicker">SELECT YOUR 48 · 61 COMPOSERS</p><h1>选择本届世界杯参赛作曲家</h1><p>可以直接使用系统默认48人，也可以从61位候选者中自选48人；选定后再进行十二音级分组抽签。</p></div><div className="roster-toolbar"><div><span>已选择 <b>{roster.length}</b> / 48</span><small>{roster.length===48?'名单已满足抽签人数':'请先取消一位默认选手，再加入候补选手'}</small></div><Button variant="outline" onClick={()=>setRoster(DEFAULT_COMPOSER_IDS)}>恢复默认48人</Button><Button className="primary-action" onClick={()=>onStart(DEFAULT_COMPOSER_IDS)}>直接使用默认名单</Button><Button className="primary-action" disabled={roster.length!==48} onClick={()=>onStart(roster)}>使用自选48人<ChevronRight/></Button></div><div className="roster-grid">{composers.map((composer,index)=><article key={composer.id} className={`roster-card ${selected.has(composer.id)?'is-selected':''} ${index>=48?'is-reserve':''}`} onClick={()=>toggle(composer.id)} role="checkbox" aria-checked={selected.has(composer.id)} tabIndex={0} onKeyDown={(event)=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();toggle(composer.id)}}}><div className="roster-portrait"><ComposerPortrait composer={composer}/>{selected.has(composer.id)&&<span><Check/></span>}</div><div><p>{index>=48?'扩展候选':'默认选手'} · {composer.period}</p><h2>{composer.nameZh}</h2><small>{composer.nameOriginal}</small><em>{composer.region}</em></div></article>)}</div><div className="roster-bottom"><span>已选择 <b>{roster.length}</b> / 48</span><Button className="primary-action" disabled={roster.length!==48} onClick={()=>onStart(roster)}>确认名单并进入抽签<ChevronRight/></Button></div></section>;
}
...
export default function Home(){
  ...
  const toggleAudio=async(composer:Composer,work:Work,index:number)=>{
    ...
    try{const source=cached||await resolveTrackSource(composer,work);await unlock;if(!cached){player.pause();player.src=source.url;player.volume=1;player.load()}setAudio((value)=>({...value,sources:{...value.sources,[trackId]:source}}));
      player.ontimeupdate=()=>setAudio((value)=>({...value,progress:Math.min(player.currentTime,30)/30}));player.onended=()=>stopAudio();player.onerror=()=>{stopAudio();setAudio((value)=>{const sources={...value.sources};delete sources[trackId];return {...value,sources,active:null,loading:null,error:trackId,errorDetail:'当前地区或音频地址不可用，已清除缓存',progress:0}})};
