...
import { ArrowLeft, Check, ChevronRight, Crown, Download, LoaderCircle, Medal, Music2, Pause, Play, RotateCcw, Shuffle, Sparkles, Swords, Trophy } from 'lucide-react';
...
type AudioState={active:string|null;loading:string|null;error:string|null;progress:number;sources:Record<string,TrackSource>};
...
async function resolveTrackSource(composer:Composer,work:Work):Promise<TrackSource>{
  ...
  for(const country of ['US','DE','GB']){
    const results=await searchItunes(query,country);
  ...
  if(!match?.previewUrl)throw new Error('No matching preview');
...
function WorkList({composer,audio,onToggle,compact=false}:{composer:Composer;audio:AudioState;onToggle:(composer:Composer,work:Work,index:number)=>void;compact?:boolean}){
  return <section className={`works ${compact?'compact-works':''}`} aria-label={`${composer.nameZh}代表作`}><div className="section-label"><Music2/>代表作 · 30秒试听</div><ul>{composer.works.map((work,index)=>{
    ...
    return <li key={trackId} className={playing?'is-playing':''}><span className="work-number">0{index+1}</span><div className="work-copy"><button className="work-title-button" onClick={()=>onToggle(composer,work,index)}><strong>{work.nameZh}</strong><small>{work.nameEn}</small></button>{failed&&<em>暂未匹配到可用试听，可稍后重试</em>}{source&&<a className="audio-credit" href={source.trackUrl} target="_blank" rel="noreferrer">{source.provider} · {source.detail}</a>}</div><button className="play-button" onClick={()=>onToggle(composer,work,index)} disabled={loading} aria-label={`${playing?'暂停':'播放'}${work.nameZh}`}>{loading?<LoaderCircle className="spin"/>:playing?<Pause/>:<Play/>}</button>{playing&&<span className="track-progress" style={{'--track-progress':`${audio.progress*100}%`} as React.CSSProperties}/>}</li>})}</ul></section>;
}

function ComposerCard({composer,selected,audio,onToggleAudio,onSelect}:{composer:Composer;selected:boolean;audio:AudioState;onToggleAudio:(composer:Composer,work:Work,index:number)=>void;onSelect:()=>void}){
  ...
  return <article className={`composer-card ${selected?'is-selected':''} ${showWorks?'is-flipped':''}`} role="checkbox" aria-checked={selected} tabIndex={0} onClick={selectFromCard} onKeyDown={selectFromKeyboard}>
    <div className="desktop-card-content"><div className="portrait-wrap"><ComposerPortrait composer={composer}/><Badge className="period-badge">{composer.period}</Badge>{selected&&<span className="selected-mark"><Check/>已入选</span>}</div><div className="card-body"><header className="composer-heading"><p className="eyebrow">{composer.years} · {composer.region}</p><h2>{composer.nameZh}</h2><p className="original-name">{composer.nameOriginal}</p></header><p className="bio">{composer.bio}</p><WorkList composer={composer} audio={audio} onToggle={onToggleAudio}/><Button size="lg" variant={selected?'outline':'default'} className="vote-button" onClick={onSelect}>{selected?<Check/>:<Trophy/>}{selected?'取消选择':'选择晋级'}</Button></div></div>
    <div className="mobile-card-content">
      <div className="mobile-card-front"><div className="mobile-portrait"><ComposerPortrait composer={composer}/>{selected&&<span className="mobile-selected-mark"><Check/></span>}</div><div className="mobile-summary"><p className="mobile-period">{composer.period}</p><h2>{composer.nameZh}</h2><p className="original-name">{composer.nameOriginal}</p><p className="mobile-region">{composer.years} · {composer.region}</p><p className="mobile-bio">{composer.bio}</p><button className="flip-card-button" onClick={()=>setShowWorks(true)}><Music2/>我想听听代表作</button></div></div>
      <div className="mobile-card-back"><header><h2>{composer.nameZh}</h2><small>{composer.nameOriginal}</small></header><WorkList composer={composer} audio={audio} onToggle={onToggleAudio} compact/><button className="flip-card-button back-button" onClick={()=>setShowWorks(false)}><RotateCcw/>返回简介</button></div>
    </div>
  </article>;
}
...
function GroupView({state,selected,onToggle,onConfirm,audio,onToggleAudio}:{state:TournamentState;selected:string[];onToggle:(id:string)=>void;onConfirm:()=>void;audio:AudioState;onToggleAudio:(composer:Composer,work:Work,index:number)=>void}){
  const group=state.groups[state.activeGroup].map((id)=>byId.get(id)!);
  return <section className="group-view"><div className="match-intro"><p className="kicker">{groupTitle(state.activeGroup)} · {state.activeGroup+1} / 12</p><h1>{GROUP_PITCHES[state.activeGroup]} 音级组 · 选择两位作曲家</h1><p>四人同时比较，可试听代表作；选满两位后确认晋级。</p></div><div className="four-grid">{group.map((composer)=><ComposerCard key={composer.id} composer={composer} selected={selected.includes(composer.id)} audio={audio} onToggleAudio={onToggleAudio} onSelect={()=>onToggle(composer.id)}/>)}</div><div className="group-confirm"><span>已选择 <b>{selected.length}</b> / 2</span><Button className="primary-action" onClick={onConfirm} disabled={selected.length!==2}>确认本组晋级<ChevronRight/></Button></div></section>;
}

function RepechageCard({composer,selected,audio,onToggleAudio,onSelect}:{composer:Composer;selected:boolean;audio:AudioState;onToggleAudio:(composer:Composer,work:Work,index:number)=>void;onSelect:()=>void}){
  return <article className={`repechage-card ${selected?'is-selected':''}`}><div className="repechage-portrait"><ComposerPortrait composer={composer}/>{selected&&<span><Check/></span>}</div><div className="repechage-copy"><p>{composer.period} · {composer.region}</p><h3>{composer.nameZh}</h3><small>{composer.nameOriginal}</small></div><WorkList composer={composer} audio={audio} onToggle={onToggleAudio} compact/><Button variant={selected?'outline':'default'} onClick={onSelect}>{selected?<><Check/>已选择</>:<>选择复活</>}</Button></article>;
}

function RepechageView({state,selected,onToggle,onConfirm,audio,onToggleAudio}:{state:TournamentState;selected:string[];onToggle:(id:string)=>void;onConfirm:()=>void;audio:AudioState;onToggleAudio:(composer:Composer,work:Work,index:number)=>void}){
  const eliminated=eliminatedComposers(state).map((id)=>byId.get(id)!);
  return <section className="repechage-view"><div className="stage-intro"><p className="kicker">REPECHAGE · 24 COMPOSERS</p><h1>从24位淘汰者中复活8位</h1><p>这是进入32强的最后机会。你可以试听曲目，然后选出8位复活。</p></div><div className="selection-status sticky-selection"><span>已选择 <b>{selected.length}</b> / 8</span><Button className="primary-action" disabled={selected.length!==8} onClick={onConfirm}>确认8位复活<ChevronRight/></Button></div><div className="repechage-grid">{eliminated.map((composer)=><RepechageCard key={composer.id} composer={composer} selected={selected.includes(composer.id)} audio={audio} onToggleAudio={onToggleAudio} onSelect={()=>onToggle(composer.id)}/>)}</div></section>;
}
...
function KnockoutCard({composer,audio,onToggleAudio,onChoose}:{composer:Composer;audio:AudioState;onToggleAudio:(composer:Composer,work:Work,index:number)=>void;onChoose:()=>void}){
  return <article className="knockout-composer"><div className="knockout-portrait"><ComposerPortrait composer={composer}/><Badge className="period-badge">{composer.period}</Badge></div><div className="knockout-copy"><p className="eyebrow">{composer.years} · {composer.region}</p><h2>{composer.nameZh}</h2><p className="original-name">{composer.nameOriginal}</p><p className="bio">{composer.bio}</p><WorkList composer={composer} audio={audio} onToggle={onToggleAudio}/><Button className="vote-button" onClick={onChoose}><Trophy/>选择晋级</Button></div></article>;
}

function KnockoutView({state,audio,onToggleAudio,onChoose,onBack}:{state:TournamentState;audio:AudioState;onToggleAudio:(composer:Composer,work:Work,index:number)=>void;onChoose:(id:string)=>void;onBack:()=>void}){
  ...
  return <section className="knockout-view"><div className="knockout-head"><div><p className="kicker">KNOCKOUT · {roundLabel(round.entrants.length)}</p><h1>{roundLabel(round.entrants.length)}</h1><p>本轮第 {knockout.currentMatch+1} / {round.matches.length} 场 · 点击作品可先试听，再选出晋级者。</p></div><div className="knockout-tools"><Button variant="outline" size="sm" onClick={onBack} disabled={knockout.currentRound===0&&knockout.currentMatch===0}><ArrowLeft/>回到上一步</Button><div className="round-meter"><span>{completed}/{round.matches.length}</span><i><b style={{width:`${completed/round.matches.length*100}%`}}/></i></div></div></div><div className="knockout-match-grid"><KnockoutCard composer={a} audio={audio} onToggleAudio={onToggleAudio} onChoose={()=>onChoose(a.id)}/><div className="knockout-vs">VS</div><KnockoutCard composer={b} audio={audio} onToggleAudio={onToggleAudio} onChoose={()=>onChoose(b.id)}/></div></section>;
}
...
export default function Home(){
  ...
  useEffect(()=>{try{const saved=restoreTournament(JSON.parse(localStorage.getItem(STORAGE_KEY)||'null'));setState(saved||createTournament())}catch{setState(createTournament())}return()=>{audioRef.current?.pause();if(fadeTimerRef.current)clearInterval(fadeTimerRef.current)}},[]);
  ...
  const toggleAudio=async(composer:Composer,work:Work,index:number)=>{
    ...
    stopAudio();setAudio((value)=>({...value,active:null,loading:trackId,error:null,progress:0}));
    try{const source=audio.sources[trackId]||await resolveTrackSource(composer,work);setAudio((value)=>({...value,sources:{...value.sources,[trackId]:source}}));const player=new Audio(source.url);audioRef.current=player;player.preload='metadata';player.volume=1;
    ...
    }catch{stopAudio();setAudio((value)=>({...value,active:null,loading:null,error:trackId,progress:0}))}
  };
  ...
  return <main className="site-shell"><header className="topbar"><a className="brand" href="#top"><span className="brand-mark"><Music2/></span><span><strong>作曲家世界杯</strong><small>COMPOSER WORLD CUP</small></span></a><div className="stage-progress"><span>{stageText}</span><i><b style={{width:`${progress}%`}}/></i></div>{state.phase!=='draw'&&state.phase!=='result'?<Button variant="ghost" size="sm" onClick={reset}><RotateCcw/>重新开始</Button>:<span/>}</header><div id="top">{state.phase==='draw'&&<DrawView state={state} onRedraw={()=>setState(createTournament())} onStart={()=>setState({...state,phase:'group-selection'})}/>} {state.phase==='group-selection'&&<GroupView state={state} selected={selected} onToggle={(id)=>togglePick(id,2)} onConfirm={confirmGroup} audio={audio} onToggleAudio={toggleAudio}/>} {state.phase==='repechage'&&<RepechageView state={state} selected={selected} onToggle={(id)=>togglePick(id,8)} onConfirm={confirmRepechage} audio={audio} onToggleAudio={toggleAudio}/>} {state.phase==='finished'&&<FinishedView state={state} onReset={reset} onStart={startKnockout}/>} {state.phase==='knockout'&&<KnockoutView state={state} audio={audio} onToggleAudio={toggleAudio} onChoose={chooseKnockoutWinner} onBack={undoKnockoutMatch}/>} {state.phase==='round-transition'&&<RoundTransitionView state={state} onContinue={continueKnockout} onBack={undoKnockoutMatch}/>} {state.phase==='result'&&<FinalResultView state={state} onReset={reset} onDownload={()=>downloadResultImage(state)}/>}</div><footer><span className="footer-music"><Music2/>肖像来自 Wikipedia · 试听由 Wikimedia Commons 与 iTunes 提供</span><span>十二平均律12组 · 8人复活 · 32强固定签表 · 决出最终冠军</span></footer></main>;
}
