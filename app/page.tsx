...
function ComposerCard({composer,selected,audio,onToggleAudio,onSelect}:{composer:Composer;selected:boolean;audio:AudioState;onToggleAudio:(composer:Composer,work:Work,index:number)=>void;onSelect:()=>void}){
  return <article className={`composer-card ${selected?'is-selected':''}`}><div className="portrait-wrap"><ComposerPortrait composer={composer}/><Badge className="period-badge">{composer.period}</Badge>{selected&&<span className="selected-mark"><Check/>已入选</span>}</div><div className="card-body"><header className="composer-heading"><p className="eyebrow">{composer.years} · {composer.region}</p><h2>{composer.nameZh}</h2><p className="original-name">{composer.nameOriginal}</p></header><p className="bio">{composer.bio}</p><WorkList composer={composer} audio={audio} onToggle={onToggleAudio}/><Button size="lg" variant={selected?'outline':'default'} className="vote-button" onClick={onSelect}>{selected?<Check/>:<Trophy/>}{selected?'取消选择':'选择晋级'}</Button></div></article>;
}
